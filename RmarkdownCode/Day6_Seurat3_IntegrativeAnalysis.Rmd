...
for (i in 1:nrow(sampleInfo)) {
  ...
  #scRNA[['condition']] <- as.character(sampleInfo[i,8])
  ## mitochondrial genes
...
scRNA.integrated <- FindNeighbors(scRNA.integrated, dims = 1:40, verbose=FALSE)
scRNA.integrated <- FindClusters(scRNA.integrated, resolution = 1)

scRNA.integrated <- RunUMAP(object = scRNA.integrated, reduction = "pca", dims = 1:40)
...
cellN<-data.frame(cbind(row.names(freq_table), table(scRNA.integrated@active.ident, scRNA.integrated@meta.data[,"sample"]), round(freq_table*100, 3), round(freq_table[,1]/freq_table[,2],2)))
...
###  `r text_spec("UMAP plots separated by sample", color = "#4d648d", font_size=20)`

```{r Seurat_plotsBySample, message=FALSE, warning=FALSE, echo=FALSE, fig.height=5, fig.width=12, results='asis'}
## don't use the seurat function

## first get the samples what have clusters missing
## otherwise the colors on the umap will be differnet between samples
cellN0.sample<-unique(as.vector(which(cellN[, 2:((ncol(cellN)+1)/2)]==0, arr.ind=TRUE)[,2]))

samplePlot<-list()

if (length(cellN0.sample)>0) {
for (i in 1:length(unique(unlist(scRNA.integrated[['sample']])))) {
	if (i %in% cellN0.sample) {
		cellN0.cluster<-which(cellN[,i+1]==0)
		samplePlot[[i]]<-DimPlot(subset(scRNA.integrated, cells=colnames(scRNA.integrated)[grep(unique(unlist(scRNA.integrated[['sample']]))[i], colnames(scRNA.integrated))]), reduction = "umap", label = TRUE,  cols=colors[1:length(unique(scRNA.integrated@active.ident))][-cellN0.cluster]) + NoLegend() + ggtitle(unique(unlist(scRNA.integrated[['sample']]))[i])
	} else {
		samplePlot[[i]]<-DimPlot(subset(scRNA.integrated, cells=colnames(scRNA.integrated)[grep(unique(unlist(scRNA.integrated[['sample']]))[i], colnames(scRNA.integrated))]), reduction = "umap", label = TRUE,  cols=colors[1:length(unique(scRNA.integrated@active.ident))]) + NoLegend() + ggtitle(unique(unlist(scRNA.integrated[['sample']]))[i])
	}
rm(cellN0.cluster)
}}

if (length(cellN0.sample)==0) {
  for (i in 1:length(unique(unlist(scRNA.integrated[['sample']])))) samplePlot[[i]]<-DimPlot(subset(scRNA.integrated, cells=colnames(scRNA.integrated)[grep(unique(unlist(scRNA.integrated[['sample']]))[i], colnames(scRNA.integrated))]), reduction = "umap", label = TRUE,  cols=colors[1:length(unique(scRNA.integrated@active.ident))]) + NoLegend() + ggtitle(unique(unlist(scRNA.integrated[['sample']]))[i])
}

plot_grid(plotlist=samplePlot, ncol=2)
```

### `r text_spec("Number of cells in each cluster", color = "#4d648d", font_size=20)`
...
brks <- quantile(c(as.numeric(as.character(cellN[, 4])), as.numeric(as.character(cellN[, 5]))), probs = seq(.05, .95, .05), na.rm = TRUE)
clrs <- round(seq(255, 40, length.out = length(brks) + 1), 0) %>%
{paste0("rgb(", ., ",", ., ",200)")}

datatable(cellN, extensions = "Buttons", options = list(bPaginate = FALSE, dom = "Bfrtip", buttons = c('copy', 'csv', 'excel', 'pdf'))) %>% formatStyle(4:ncol(cellN), backgroundColor = styleInterval(brks, clrs))
```
...
## set Default assay to RNA for the following analysis
...
uniqueC<-as.numeric(as.character(uniqueC))[order(as.numeric(as.character(uniqueC)))]
...
sampleN1<-nrow(sampleInfo)+1
smallCR<-unlist(sapply(2:sampleN1, function(i) which(as.numeric(as.character(cellN[,i]))<3)))
smallCR<-unique(smallCR)[order(unique(smallCR))]
smallC<-as.numeric(as.character(cellN[smallCR, 1]))
smallC<-smallC[order(smallC)]


if (length(smallC)>0) {
    largeC<-uniqueC[-unlist(sapply(smallC, function(i) which(uniqueC==i, arr.ind=TRUE)))]
    largeC<-as.numeric(as.character(largeC))
    largeC<-largeC[order(largeC)]}
if (length(smallC)==0) largeC<-uniqueC
...
if (length(smallC)>0) {
       # ct.markers<-list()
        for (i in largeC) {
                k <- i+1
                ct.markers[[k]]<-FindConservedMarkers(scRNA.integrated, ident.1 = i, assay.type = "RNA", grouping.var = "sample")
                ct.markers[[k]]$avg_logFC<-rowMeans(ct.markers[[k]][grep("_avg_logFC", colnames(ct.markers[[k]]))])
                ct.markers[[k]]$p_val<-ct.markers[[k]]$max_pval
        }
        for (p in smallC) {
                m <- p+1
                ct.markers[[m]]<-FindMarkers(scRNA.integrated, ident.1 = p, min.pct = 0.25, logfc.threshold = 0.25)
       }
}
...
## cluster have more than 0 marker genes
cde<-which(sapply(ct.markers, nrow)>0, arr.ind=TRUE)
clusterName<-paste0("cluster", "", cde-1)

## positive markers
ct.markers.pos<-lapply(seq_along(ct.markers), function(i) subset(ct.markers[[i]], avg_logFC>0))

## violin plots of top 9 cluster markers
vlnP<-lapply(cde, function(i) VlnPlot(scRNA.integrated, features= head(row.names(ct.markers.pos[[i]]), 9), ncol=3, cols=colors[1:length(unique(scRNA.integrated@active.ident))]) + theme(legend.position="none"))

## feature plots of top 9 cluster markers
feaP<-lapply(cde, function(i) FeaturePlot(scRNA.integrated, features= head(row.names(ct.markers.pos[[i]]), 9), cols=c("grey", "firebrick1"), reduction = "umap", ncol=3))

## violin plots of top 9 cluster markers separated by sample
vlnPS1<-lapply(cde, function(i) VlnPlot(scRNA.integrated, features= head(row.names(ct.markers.pos[[i]]), 9), split.by="sample", pt.size = 0, cols=c("grey", "firebrick1"), combine = FALSE))
vlnPS2<-list()
for (i in cde) vlnPS2[[i]]<-lapply(seq_along(vlnPS1[[i]]), function(j) vlnPS1[[i]][[j]] + xlab(""))
## combine the plots
...
feaHM<-lapply(cde, function(i) FeaturePlot(scRNA.integrated, features= head(row.names(ct.markers.pos[[i]]), 9), cols=c("grey", "firebrick1"), split.by = "sample"))

##volcano plots of all cluster marker genes
source("../scripts/VolcanoSC_integrativeAnalysis.r")
VolcanoT<-paste0("Volcano plot for cluster ", cde-1, " marker genes")
VolcanoP<-lapply(cde, function(i) VolcanoSC(ct.markers[[i]], 1e-10, 0.25, 1e-10, VolcanoT[i]))
```

## Data visulization

### `r text_spec("Violin and feature plots ", color = "#4d648d", font_size=16)`
...
### `r text_spec("Feature plots separated by condition", color = "#4d648d", font_size=16)`

#### Feature plots for conserved marker genes of each cluster separated by sample/condition/treatment {.tabset .tabset-fade}

```{r Seurat_CellTypeMarkers_vlnp, message=FALSE,warning=FALSE, echo=FALSE, fig.height=20, fig.width=12, results="asis"}
#clusterName<-paste0("cluster", "", uniqueC)
...
if (ref=="hg38") {
        genes <- lapply(cde, function(i) select(Homo.sapiens, keys=row.names(ct.markers[[i]]), columns=c("ENTREZID", "GENENAME", "TXCHROM"), keytype="SYMBOL"))
        pathEnrich<-lapply(seq_along(genes), function(i) enrichPathway(gene=genes[[i]][,2], pvalueCutoff=0.5, readable=T))
}

if (ref=="mm10") {
        genes <- lapply(cde, function(i) select(Mus.musculus, keys=row.names(ct.markers[[i]]), columns=c("ENTREZID", "GENENAME", "TXCHROM"), keytype="SYMBOL"))
        pathEnrich<-lapply(seq_along(genes), function(i) enrichPathway(gene=genes[[i]][,2], organism = "mouse", pvalueCutoff=0.5, readable=T))
}

dotP<-lapply(seq_along(genes), function(i) dotplot(pathEnrich[[i]], showCategory=30, title=paste0("cluster", "", (cde-1))))
```
...
```{r Seurat_CellTypeMarkers_rmData, message=FALSE,warning=FALSE, echo=FALSE, results="hide"}
rm(vlnP, feaP, feaHM, VolcanoP)
```

### `r text_spec("Heatmap of each sample", color = "#4d648d", font_size=16)`


```{r Seurat_HeatMap_plot1, message=FALSE,warning=FALSE, echo=FALSE, fig.height=5, fig.width=6, results="asis"}
## genes tp label from the users
sig.genes.label1<-c("LEF1", "DSP", "KRT19", "KRT13", "KRT17", "IGF1", "VIM", "LUM", "ELN", "CDK1", "CENPF", "COL6A3", "MATN1", "KRT4", "CLIC3", "EDNRB", "PLP1", "POSTN", "TRIL", "APCDD1", "TWIST2")
...
for (i in seq_along(ct.markers)[-1]) {
...
ct.markers.sigAll<-do.call(rbind, ct.markers.sig)
...
```{r Seurat_HeatMap_plot2, message=FALSE,warning=FALSE, echo=FALSE, fig.height=5, fig.width=6, results="asis"}
...
nms1<-paste0(largeC, "_", unique(scRNA.integrated[[]]$sample)[1])
nms2<-paste0(largeC, "_", unique(scRNA.integrated[[]]$sample)[2])
nmsp1<-unlist(sapply(seq_along(nms1), function(i) which(names(cluster.averages)==nms1[i], arr.ind=TRUE)))
nmsp2<-unlist(sapply(seq_along(nms2), function(i) which(names(cluster.averages)==nms2[i], arr.ind=TRUE)))

## further process the average expression matrix and add color information
cluster.aexp<-list()
geneN<-c()
for (i in seq_along(largeC)) {
...
tt<-sapply(seq_along(largeC), function(i) paste(paste("cluster", i-1, sep=" "), "average gene expression", sep=" "))

p<-lapply(seq_along(largeC), function(i) ggplot(cluster.aexp[[i]], aes(cluster.aexp[[i]][,1], cluster.aexp[[i]][,2], label=gene)) + geom_point(color = cluster.aexp[[i]]$color, size=0.5) + geom_text_repel(color=cluster.aexp[[i]]$color, segment.color = "grey50", size=2, box.padding = unit(0.35, "lines"), point.padding = unit(0.3, "lines")) + ggtitle(tt[[i]]) + xlab(names(cluster.aexp[[i]])[1]) + ylab(names(cluster.aexp[[i]])[2]))
```
...
clusterName<-paste0("cluster", "", largeC)
for (i in 1:length(p)) {
  cat("##### ",clusterName[i],"\n")
  print(p[[i]])
  #print(pHigh[[i]])
...
cluster.DE <- lapply(seq_along(largeC), function(i) FindMarkers(scRNA.integrated, ident.1 = uniqueC1[i], ident.2 = uniqueC2[i], verbose = FALSE))
for (i in 1:length(cluster.DE)) cluster.DE[[i]]$gene<-row.names(cluster.DE[[i]])
cluster.DEsg <- lapply(seq_along(largeC), function(i) subset(cluster.DE[[i]], p_val<0.05))
```
...
clusterDEMG<-lapply(seq_along(largeC), function(i) setdiff(uMG, row.names(cluster.DEsg[[i]])))
## get the marker genes for each cluster including the ones not for the cluster
clusterMG<-lapply(seq_along(largeC), function(i) rbind(cluster.DEsg[[i]][, c(6, 2, 5)], data.frame(gene=clusterDEMG[[i]], avg_logFC="", p_val_adj="")))
...
colnames(clusterMGall)[2:ncol(clusterMGall)]<-paste(rep(paste("cluster", largeC, sep=""), each=2), c("avg_logFC", "p_val_adj"), sep=":")
...
#saveRDS(scRNA.integrated, file = "DSP_d6_WA25_d6_IntegrativeAnalysis.rds")
## save cluster-specific marker genes
#saveRDS(ct.markers, file = "DSP_d6_WA25_d6_IntegrativeAnalysis_clusterMarkerGenes.rds")
```
